...
if (empty($_SESSION['loggedIn'])) {
    ...
    $guestCheck = true;
    $success = false;
    $errorMsg = "You are require to login in order to view this webpage";
}
...
if (isset($_SESSION['role'])) {
    if ($_SESSION['role'] == 'admin') {
        $adminCheck = true;
        $success = false;
        $errorMsg = "Admin had nothing to add to cart";
    }
}
if ($success) {
    include 'dbinfo.php';
...
//if ($adminCheck || $guestCheck || $memberCheck) {
//    echo $errorMsg;
//} else {
//    echo "Got Cart";
//}
...
<html lang="en">
    ...
    <body>
                        ...
                        <div class='card mb-3 text-center'>
                            <div class='row align-items-center'>
                                <div class="col-lg-2 col-md-3 col-sm-12">
                                    <label>
                                        <input type="checkbox" class="amtCheckBox" name="<?php echo $clothings[$i]['clothing_id'] ?>" value="<?php echo $clothings[$i]['total_price'] ?>" >
                                    </label>
                                </div>
                                <?php $withoutExt = preg_replace('/\\.[^.\\s]{3,4}$/', '', $row['clothing_image']); ?>
                                <div class="col-lg-4 col-md-3 col-sm-12"><img src="<?php echo 'images/' . $clothings[$i]['clothing_image'] ?>" alt='<?php echo $withoutExt; ?>' style="max-width: 50%" class="clothing-image"></div>
                                <div class="col-lg-4 col-md-3 col-sm-12">
                                    <div><input type='hidden' class='ce_id' value='<?php echo $clothings[$i]['cart_id'] ?>' ></div>
                                    <div class="ce_type" data-type-id="<?php echo $clothings[$i]['clothing_type'] ?>">Type: <?php echo $clothings[$i]['clothing_type'] ?></div>
                                    <div class="ce_name" data-name="<?php echo $clothings[$i]['clothing_title'] ?>">Name: <?php echo $clothings[$i]['clothing_title'] ?></div>
                                    <div class="ce_price" data-price="<?php echo number_format((float) $clothings[$i]['total_price'], 2, '.', '') ?>">Price: $<?php echo number_format((float) $clothings[$i]['total_price'], 2, '.', '') ?></div>
                                    <div class="ce_size">Size: <?php echo $clothings[$i]['size_name'] ?></div>
                                    <div class="ce_color">Color: <?php echo $clothings[$i]['color_name'] ?></div>
                                    <div class="ce_qty" data-qty="<?php echo $clothings[$i]['qty'] ?>">Qty: <?php echo $clothings[$i]['qty'] ?></div>
                                </div>
                                <div class="col-lg-2 col-md-3 col-sm-12">
            <!--                            <button class="btn btn-success" id="<?php echo $clothings[$i]['clothing_id'] ?>">Edit</button>-->
                                    <button class="btn btn-success btn-edit-cart" data-cart-id="<?php echo $clothings[$i]['cart_id'] ?>" data-clothing-id="<?php echo $clothings[$i]['clothing_id'] ?>" data-color-id="<?php echo $clothings[$i]['color_id'] ?>" data-toggle="modal" data-target="#editCartModel" data-size-id=<?php echo $clothings[$i]['size_id'] ?>><span class="fas fa-pencil-alt"></span>Edit</button>
                                    <button class="btn btn-danger btn-delete-cart" data-toggle="modal" data-target="#deleteCartModel" data-cart-id="<?php echo $clothings[$i]['cart_id'] ?>" data-c-id="<?php echo $clothings[$i]['clothing_id'] ?>" data-size-id=<?php echo $clothings[$i]['size_id'] ?>><span class="fas fa-trash-alt"></span>Delete</button>
                                </div>
                            </div>
                        </div>
                    <?php } ?>
                    ...
                    <div class='row mb-3'>
                        ...
                        <!--<button class='btn btn-danger ml-auto btn-check-out'>Checkout</button>-->
        ...
        <div class="modal fade" id="editCartModel" tabindex="-1" role="dialog" aria-labelledby="editCartModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-success" id="edit_to_cart_ajax"><span class="fas fa-pencil-alt"></span>Edit Cart</button>
                    </div>
                </div>
            </div>
        </div>

        <div class="modal fade" id="deleteCartModel" tabindex="-1" role="dialog" aria-labelledby="deleteCartModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="deleteCartModalLabel">Delete Cart</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
